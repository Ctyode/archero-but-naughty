...
function generateSkillTree() {
  ...
  for (var item in tree.skills) {
      ...
      if(tree.skills[item].super_parent == "basic") {
        ...
        if(tree.skills[item].base_child == 1) {
        ...
        if(tree.skills[item].children == 2) {
      ...
      if(tree.skills[item].children != null) {
        for (let index = 0; index < tree.skills[item].children; index++) {
      ...
      if(tree.skills[item].super_parent != "basic") {
        let element = document.getElementById('line' + tree.skills[item].line);
        element.appendChild(skill_item);

        if(tree.skills[item].children == 1) {
          let border_center_horizontal_short = document.createElement('div');
          border_center_horizontal_short.classList.add('border_center_horizontal_short')
      ...
      icon.className = `icon`;
      icon.src = tree.skills[item].icon;
      ...
      image.src = tree.skills[item].image;

      let title = document.createElement('div');
      title.classList.add("info_title");
      title.innerHTML = tree.skills[item].title;

      let description = document.createElement('div');
      description.classList.add("info_description");
      description.innerHTML = tree.skills[item].description;


      let buffs = document.createElement('div');
      buffs.classList.add("info_buffs");

      Object.keys(tree.skills[item].buffs_description).forEach(function(k){
        console.log(k + ' - ' + tree.skills[item].buffs_description[k]);

        let buff =  document.createElement('div');
        buff.classList.add("info_buff_description");
        buff.innerHTML = tree.skills[item].buffs_description[k];
      ...
      cost.innerHTML = "Cost: " + tree.skills[item].cost;
